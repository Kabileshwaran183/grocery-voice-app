export default function ItemList({ items, handleDeleteItem }) {
  if (items.length === 0) return (
    <div className="text-center py-8">
      <p className="text-lg text-neutral-600 mb-2">No items added yet</p>
      <p className="text-sm text-neutral-500">Use the microphone buttons above</p>
    </div>
  );
  ...
  return (
    <ul className="space-y-3">
      {items.map((item, idx) => (
        <li
          key={idx}
          className="border border-neutral-200 p-2  rounded-lg shadow-sm 
                      flex justify-between items-center  bg-white hover:bg-neutral-50
